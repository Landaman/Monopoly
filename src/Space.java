...
public class Space {
     ...
     */
    public Space(String name, int moneyLoss, int movementLoss, int spaceLoss, String colorGroup, int perHouse,
                 int perHotel, int deckUsed, int boardSize, int numDecks, Property property) {
        if (name != null && name.length() > 0 && spaceLoss >= -1 && deckUsed >= -1 && deckUsed < numDecks &&
                spaceLoss < boardSize && !((perHouse != 0 && perHotel == 0) || (perHouse == 0 && perHotel != 0)) &&
